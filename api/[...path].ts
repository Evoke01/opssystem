...
function normalizeWeekOffs(value: unknown) {
...
function dateKeyWeekday(dateKey: string): WeekdayName {
  return WEEKDAY_NAMES[parseDateKey(dateKey).getUTCDay()];
}

function eachDate(start: string, endExclusive: string) {
  const dates: string[] = [];
  let cursor = start;
...
function zonedNowInfo(now: Date, timeZone: string) {
...
function reportTotals(report: Partial<ReportRow> | null | undefined) {
...
function isWorkingDay(dateKey: string, user: ScheduleUser, leaveDates: Set<string>) {
...
function countWorkingDays(start: string, endExclusive: string, user: ScheduleUser, leaveDates: Set<string>) {
  let workingDays = 0;
  let leaveDays = 0;
  let weekOffDays = 0;
